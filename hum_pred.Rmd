...
# packages to install: renv, dplyr, readr, rphylopic, forcats, tidyr, cowplot (loads ggplot), jpeg
...
# cowplot::save_plot("figs/fig1_use_threat.png", fig1_log, base_height = 8, base_width = 14, dpi = 600)
...
# function to summarise use data per realm and class
use_realm_class <- function(realm, class) {
  
  out <- use_raw %>% 
    dplyr::filter(!!sym(realm) == TRUE & class_name == class) %>% 
    dplyr::summarise_at(vars(starts_with("useTrade")), sum) %>% 
    tidyr::gather("useTradeId", "freq", useTradeId.19:useTradeId.31) %>% 
    dplyr::left_join(use_names, by = "useTradeId") %>% 
    dplyr::arrange(-freq)
...
use_class <- function(class) {
  ...
  out2 <- use_raw %>% 
    dplyr::filter(class_name == class) %>% 
    dplyr::select(out$useTradeId[1:3]) %>% 
    #dplyr::select(out$useTradeId) %>% 
    setNames(out$use_name_short[1:3]) %>% 
    #setNames(out$use_name_short) %>% 
...
Here we calculate the top uses per realm and create the plot for 2
...
top_realms <- lapply(realms, use_realm) %>% 
  dplyr::bind_rows(.) %>% 
  dplyr::mutate(realm = factor(realm, levels = realms))

top_terr <- top_realms %>% 
  dplyr::filter(realm == "Terrestrial") %>% 
  dplyr::summarise_at(vars(-realm), sum)

# 5731/7722
# 3047/7722
# 619/7722

fit_realms <- eulerr::euler(top_realms, by = realm, shape = "ellipse")

euler_realms <- plot(fit_realms, quantities = TRUE, labels = list(font = 0.9), edges = "grey20", fills = list(fill = c("#56B4E9", "#E69F00", "#CC79A7")))

# cowplot::save_plot("outputs/euler2.png", euler_realms, base_height = 4, base_width = 11, dpi = 300)

# cowplot::save_plot("outputs/euler2.tiff", euler_realms, base_height = 4, base_width = 11, dpi = 300)

# cowplot::save_plot("outputs/euler2.pdf", euler_realms, base_height = 4, base_width = 11, dpi = 600)

```

## for each class individually

## Euler for uses across class

Here we calculate the top uses per class
...
top_class <- lapply(classes, use_class) #%>% 
  #dplyr::bind_rows(.) #%>% 
  #dplyr::select(Food, Pets, Collection, Medicine, Clothes, class_name)

# food, pets, collection, medicine, clothes

gen_euler_class <- function(df, fill) {
...
eu_act <- gen_euler_class(dplyr::select(top_class[[1]], -class_name), fill = c(food, pets, collect)) %>%
  cowplot::plot_grid(title_act, ., nrow = 2, rel_heights = c(0.1, 1))

eu_amph <- gen_euler_class(dplyr::select(top_class[[4]], -class_name), fill = c(pets, food, medi)) %>%
  cowplot::plot_grid(title_amph, ., nrow = 2, rel_heights = c(0.1, 1))

eu_aves <- gen_euler_class(dplyr::select(top_class[[6]], -class_name), fill = c(pets, food, collect)) %>%
  cowplot::plot_grid(title_aves, ., nrow = 2, rel_heights = c(0.1, 1))

eu_chon <- gen_euler_class(dplyr::select(top_class[[5]], -class_name), fill = c(food, pets, clothes)) %>%
  cowplot::plot_grid(title_chon, ., nrow = 2, rel_heights = c(0.1, 1))

eu_mam <- gen_euler_class(dplyr::select(top_class[[3]], -class_name), fill = c(food, clothes, medi)) %>%
  cowplot::plot_grid(title_mam, ., nrow = 2, rel_heights = c(0.1, 1))

eu_rept <- gen_euler_class(dplyr::select(top_class[[2]], -class_name), fill = c(pets, food, medi)) %>%
...
eu_terr <- gen_euler_class(dplyr::select(top_realms_list[[1]], -realm), fill = c(pets, food, collect)) %>%
  cowplot::plot_grid(title_terr, ., nrow = 2, rel_heights = c(0.1, 1))

eu_fresh <- gen_euler_class(dplyr::select(top_realms_list[[2]], -realm), fill = c(food, pets, collect)) %>%
  cowplot::plot_grid(title_fresh, ., nrow = 2, rel_heights = c(0.1, 1))

eu_mar <- gen_euler_class(dplyr::select(top_realms_list[[3]], -realm), fill = c(food, pets, collect)) %>%
...
# cowplot::save_plot("outputs/fig2_com_600.png", fig2_com, base_height = 10, base_width = 12, dpi = 600)

```

Here we calculate the top uses per class and realm

```{r}

combi <- list(c("Freshwater", "Actinopterygii"), c("Marine", "Actinopterygii"), c("Terrestrial", "Amphibia"), c("Freshwater", "Amphibia"), c("Terrestrial", "Aves"), c("Freshwater", "Aves"), c("Marine", "Aves"), c("Freshwater", "Chondrichthyes"), c("Marine", "Chondrichthyes"), c("Terrestrial", "Mammalia"), c("Freshwater", "Mammalia"), c("Marine", "Mammalia"), c("Terrestrial", "Reptilia"), c("Freshwater", "Reptilia"), c("Marine", "Reptilia"))

top_realm_class <- lapply(1:length(combi), function(x) {
  
  use_realm_class(realm = combi[[x]][1], class = combi[[x]][2])
  
})

gen_euler <- function(df) {
  
  # fit euler
  fit <- eulerr::euler(df, by = realm, shape = "ellipse")
  
  # plot euler
  eul <- plot(fit, quantities = TRUE, fill = "white", labels = list(font = 1))
  
  return(eul)
  
}

plot_title <- function(name, style = "bold", x = 0.4) {
...
top_act <- dplyr::bind_rows(top_realm_class[[1]], top_realm_class[[2]]) %>% 
  dplyr::mutate_at(vars(-realm), ~ifelse(is.na(.), FALSE, .)) %>% 
  dplyr::mutate(realm = factor(realm, levels = realms[2:3]))

title_act <- plot_title(name = "Actinopterygii")

euler_act <- gen_euler(top_act) %>%
  cowplot::plot_grid(title_act, ., nrow = 2, rel_heights = c(0.1, 1))

## Amphibia

top_amph <- dplyr::bind_rows(top_realm_class[[3]], top_realm_class[[4]]) %>% 
  dplyr::mutate_at(vars(-realm), ~ifelse(is.na(.), FALSE, .)) %>% 
  dplyr::mutate(realm = factor(realm, levels = realms[1:2]))

title_amph <- plot_title(name = "Amphibia")

euler_amph <- gen_euler(top_amph) %>% 
  cowplot::plot_grid(title_amph, ., nrow = 2, rel_heights = c(0.1, 1))

## Aves

top_aves <- dplyr::bind_rows(top_realm_class[[5]], top_realm_class[[6]], top_realm_class[[7]]) %>% 
  dplyr::mutate_at(vars(-realm), ~ifelse(is.na(.), FALSE, .)) %>% 
  dplyr::mutate(realm = factor(realm, levels = realms))

title_aves <- plot_title(name = "Aves")

euler_aves <- gen_euler(top_aves) %>% 
  cowplot::plot_grid(title_aves, ., nrow = 2, rel_heights = c(0.1, 1))

## Chondrichthyes

top_chon <- dplyr::bind_rows(top_realm_class[[8]], top_realm_class[[9]]) %>% 
  dplyr::mutate_at(vars(-realm), ~ifelse(is.na(.), FALSE, .)) %>% 
  dplyr::mutate(realm = factor(realm, levels = realms[2:3]))

title_chon <- plot_title(name = "Chondrichthyes")

euler_chon <- gen_euler(top_chon) %>% 
  cowplot::plot_grid(title_chon, ., nrow = 2, rel_heights = c(0.1, 1))

## Mammalia

top_mam <- dplyr::bind_rows(top_realm_class[[10]], top_realm_class[[11]], top_realm_class[[12]]) %>% 
  dplyr::mutate_at(vars(-realm), ~ifelse(is.na(.), FALSE, .)) %>% 
  dplyr::mutate(realm = factor(realm, levels = realms))

title_mam <- plot_title(name = "Mammalia")

euler_mam <- gen_euler(top_mam) %>% 
  cowplot::plot_grid(title_mam, ., nrow = 2, rel_heights = c(0.1, 1))

## Reptilia
# doesn't work in normal way - something to do with matrix under euler()

# top_rept <- dplyr::bind_rows(top_realm_class[[13]], top_realm_class[[14]], top_realm_class[[15]]) %>% 
#   dplyr::mutate_at(vars(-realm), ~ifelse(is.na(.), FALSE, .)) %>% 
#   dplyr::mutate(realm = factor(realm, levels = realms)) %>% 
#   dplyr::select(Pets, Food, Medicine, Clothes, `Animal food`, realm)
# 
# title_rept <- plot_title(name = "Reptilia")
# 
# euler_rept <- gen_euler(top_rept) %>% 
#   cowplot::plot_grid(title_rept, ., nrow = 2, rel_heights = c(0.1, 1))

marg_rept_terr <- (1 - (764 / 764)) / 2
marg_rept_fre <- (1 - (71 / 764)) / 2
marg_rept_mar <- (1 - (24 / 764)) / 2

title_rept_terr <- plot_title("Terrestrial", style = "bold.italic", x = 0.3)

euler_rept_terr <- gen_euler(top_realm_class[[13]]) %>% 
  cowplot::plot_grid(title_rept_terr, ., nrow = 2, rel_heights = c(0.1, 1))

title_rept_fre <- plot_title("Freshwater", style = "bold.italic", x = 0.3)

euler_rept_fre <- gen_euler(top_realm_class[[14]]) %>% 
  cowplot::plot_grid(NULL, NULL, NULL, NULL, ., NULL, NULL, NULL, NULL, ncol = 3, nrow = 3, rel_widths = c(marg_rept_fre, 1, marg_rept_fre), rel_heights = c(marg_rept_fre, 1, marg_rept_fre)) %>% 
  cowplot::plot_grid(title_rept_fre, ., nrow = 2, rel_heights = c(0.1, 1))

title_rept_mar <- plot_title("Marine", style = "bold.italic", x = 0.3)

euler_rept_mar <- gen_euler(top_realm_class[[15]]) %>% 
  cowplot::plot_grid(NULL, NULL, NULL, NULL, ., NULL, NULL, NULL, NULL, ncol = 3, nrow = 3, rel_widths = c(marg_rept_mar, 1, marg_rept_mar), rel_heights = c(marg_rept_mar, 1, marg_rept_mar)) %>% 
  cowplot::plot_grid(title_rept_mar, ., nrow = 2, rel_heights = c(0.1, 1))

title_rept <- plot_title(name = "Reptilia")

euler_rept <- cowplot::plot_grid(NULL, euler_rept_terr, euler_rept_fre, euler_rept_mar, NULL, ncol = 5, rel_widths = c(0.5, 1, 1, 1, 0.5)) %>% 
    cowplot::plot_grid(title_rept, ., nrow = 2, rel_heights = c(0.1, 1))

## Combine taxa

euler_taxa <- cowplot::plot_grid(euler_act, euler_amph, euler_aves, euler_chon, euler_mam, euler_rept, nrow = 6)

# # save: euler_taxa
# cowplot::save_plot("outputs/euler_taxa.png", euler_taxa, base_width = 7, base_height = 14)
...
gl_birds <- readr::read_csv("data/cobi13486-sup-0004-tables4.csv")

gl_mam <- readRDS("data/df_trait.rds") %>% 
...
#   dplyr::mutate(diet_herb = cooke2019diet_fruit + cooke2019diet_nect + cooke2019diet_seed + cooke2019diet_planto) %>% 
#   dplyr::select(binomial, diet_herb)
...
diet_pco <- readRDS("data/diet_pco.rds")
...
S <- cov(diet_all, points_stand) # loadings
...
# # save: pcoa_diet
# cowplot::save_plot("outputs/pcoa_diet.png", pcoa_diet, base_height = 6)
...
# save: list_imp_mam
saveRDS(list_imp_mam, "data/list_imp_mam.rds")
...
#tr_birds <- readRDS("data/tr_birds.rds")
...
# save: list_imp_birds
saveRDS(list_imp_birds, "data/list_imp_birds.rds")
...
list_imp_mam <- readRDS("data/list_imp_mam.rds")
list_imp_birds <- readRDS("data/list_imp_birds.rds")
...
#write.csv(pc_mean, file = "data/pc.csv", row.names = FALSE)

pc_mean <- read.csv("data/pc.csv")

# pc_dich <- pc_mean %>% 
#   dplyr::select(binomial, Comp.1, Comp.2) %>% 
#   dplyr::left_join(., dplyr::select(hum_vert, binomial = scientific_name, anyUseTrade, ext, threat_use)) %>% 
#   dplyr::mutate(threat_ext = ifelse(ext == TRUE & threat_use == TRUE, "TRUE", "FALSE"),
#                 used = ifelse(anyUseTrade == TRUE, "Used", "Not used"),
#                 threat = ifelse(anyUseTrade == TRUE & threat_ext == TRUE, "Threatened by use",
#                                 ifelse(anyUseTrade == TRUE & threat_ext == FALSE, "Used but not\nthreatened by use", "Not used"))) %>% 
#   dplyr::mutate(threat = factor(threat, levels = c("Not used", "Used but not\nthreatened by use", "Threatened by use")))
...
pca_used <- ggplot(cont_nused[[1]], aes(x = Var1, y = Var2)) +
  # add arrows
  geom_segment(data = pcload_sc, aes(x = 0, y = 0, xend = Comp.1, yend = Comp.2), arrow = arrow(length = unit(0.2, "cm")), colour = "black") +
  geom_segment(data = pcload_sc, aes(x = 0, y = 0, xend = -Comp.1, yend = -Comp.2), lty = 5, colour = "darkgrey") +
  # add species
  geom_point(data = pc_dich, aes(colour = grp, x = Comp.1, y = Comp.2), size = 0.03, alpha = 0.8) +
  # add arrow labels
  geom_text(data = pcload_sc, aes(x = Comp.1, y = Comp.2, label = trait), size = 4, nudge_x = c(0, 0, 0, 0, -0.6), nudge_y = c(0.3, -0.2, -0.4, 0.3, -0.3)) +
  geom_contour(data = cont_nused[[1]], aes(z = value), breaks = cont_nused[[2]], colour = "grey67", size = 1.4) +
  geom_contour(data = cont_used[[1]], aes(z = value), breaks = cont_used[[2]], colour = "#fd8d3c", size = 1.4) +
  geom_contour(data = cont_thrt[[1]], aes(z = value), breaks = cont_thrt[[2]], colour = "#a50f15", size = 1.4) +
  coord_equal() +
  # axis labels
  labs(x = "PC1 (38.8%)", y = "PC2 (24.2%)", colour = "", alpha = "") +
  # change size of point in legend
  guides(colour = guide_legend(override.aes = list(size = 2, alpha = 1))) + 
  scale_colour_manual(values = c("grey", "#fd8d3c", "#a50f15"), labels = c("Not used (n = 10,169)", "Used (n = 5,268 + 976)", "Extinction risk & use\nconsidered threat (n = 976)")) +
  theme(legend.position = c(0.62, 0.95))

# cowplot::save_plot("outputs/pca_threat.png", pca_threat, base_width = 10, base_height = 6, dpi = 300)
...
hyp <- function(used_col, used_dich) {
  ...
  # double default number of samples per point
  # samples.per.point = 2 * ceiling((10^(3 + sqrt(ncol(pc_used))))/nrow(pc_used)) 
  # do this if I need to reduce stochasticity
...
# 
# saveRDS(hyp_nused, "data/hyp_nused.rds")
# saveRDS(hyp_used, "data/hyp_used.rds")
# saveRDS(hyp_thrt, "data/hyp_thrt.rds")

hyp_nused <- readRDS("data/hyp_nused.rds")
hyp_used <- readRDS("data/hyp_used.rds")
hyp_thrt <- readRDS("data/hyp_thrt.rds")
...
# random samples with equivalent number of species
...
#   dplyr::bind_rows(.)
# 
# saveRDS(rn_used, "data/rn_used_glob_333.rds")
# 
# rm(rn_used); gc()
...
#   dplyr::bind_rows(.)
# 
# saveRDS(rn_thrt, "data/rn_thrt_glob_333.rds")

# volume

rn_used <- readRDS("data/rn_used_glob_333.rds")

rn_thrt <- readRDS("data/rn_thrt_glob_333.rds")

rn_vol <- dplyr::bind_rows(rn_used %>% tidyr::gather(obs_rnd, vol, obs_vol:rnd_vol) %>% dplyr::mutate(grp = "Used"), rn_thrt %>% tidyr::gather(obs_rnd, vol, obs_vol:rnd_vol) %>% dplyr::mutate(grp = "Extinction risk & use\nconsidered threat")) %>% 
...
rang_vol <- data.frame(obs_rnd = rep(c("obs_vol", "rnd_vol"), 2), vol = c(mean(rn_used$obs_vol), median(rn_used$rnd_vol), median(rn_thrt$obs_vol), mean(rn_thrt$rnd_vol)), ymin = c(quantile(rn_used$obs_vol, 0.025), quantile(rn_used$rnd_vol, 0.025), quantile(rn_thrt$obs_vol, 0.025), quantile(rn_thrt$rnd_vol, 0.025)), ymax = c(quantile(rn_used$obs_vol, 0.975), quantile(rn_used$rnd_vol, 0.975), quantile(rn_thrt$obs_vol, 0.975), quantile(rn_thrt$rnd_vol, 0.975)), grp = rep(c("Used", "Extinction risk & use\nconsidered threat"), each = 2)) %>% 
    dplyr::mutate(grp = factor(grp, levels = c("Used", "Extinction risk & use\nconsidered threat")))

lin_vol <- data.frame(yintercept = c(mean(rn_used$rnd_vol), mean(rn_thrt$rnd_vol)), grp = c("Used", "Extinction risk & use\nconsidered threat"))

vol_plot <- ggplot(data = rn_vol, aes(x = obs_rnd, y = vol)) +
  # keep plot layout
  geom_point(colour = "white") +
  geom_hline(data = lin_vol, aes(yintercept = yintercept), lty = 2, colour = c("#fd8d3c", "#a50f15")) +
  # half violin
  geom_half_violin(data = dplyr::filter(rn_vol, obs_rnd == "rnd_vol"), aes(fill = grp), side = "r", colour = NA, scale = "count", alpha = 0.4) +
  #geom_half_point(data = dplyr::filter(rn_vol, obs_rnd == "rnd_vol"), aes(colour = grp), side = "r", alpha = 0.1) +
  # pointrange
  geom_pointrange(data = rang_vol, aes(ymin = ymin, ymax = ymax), pch = 15, colour = c("#fd8d3c", "#fd8d3c", "#a50f15", "#a50f15"), lwd = 1) +
  scale_colour_manual(values = c("#fd8d3c", "#a50f15")) +
  scale_fill_manual(values = c("#fd8d3c", "#a50f15")) +
  scale_x_discrete(labels = c("Observed", "Random")) +
  labs(y = bquote("Volume "~(SD^5))) +
  facet_wrap(~grp) +
...
rang_uni <- data.frame(obs_rnd = rep(c("frac_unique_1", "frac_unique_2"), 2), uni = c(median(rn_used$frac_unique_1), median(rn_used$frac_unique_2), mean(rn_thrt$frac_unique_1), mean(rn_thrt$frac_unique_2)), ymin = c(quantile(rn_used$frac_unique_1, 0.025), quantile(rn_used$frac_unique_2, 0.025), quantile(rn_thrt$frac_unique_1, 0.025), quantile(rn_thrt$frac_unique_2, 0.025)), ymax = c(quantile(rn_used$frac_unique_1, 0.975), quantile(rn_used$frac_unique_2, 0.975), quantile(rn_thrt$frac_unique_1, 0.975), quantile(rn_thrt$frac_unique_2, 0.975)), grp = rep(c("Used", "Extinction risk & use\nconsidered threat"), each = 2)) %>% 
    dplyr::mutate(grp = factor(grp, levels = c("Used", "Extinction risk & use\nconsidered threat"))) %>% 
  dplyr::mutate_at(vars(uni, ymin, ymax), ~ . * 100)

lin_uni <- data.frame(yintercept = c(mean(rn_used$frac_unique_2), mean(rn_thrt$frac_unique_2)), grp = c("Used", "Extinction risk & use\nconsidered threat")) %>% 
  dplyr::mutate(yintercept = yintercept * 100)

uni_plot <- ggplot(data = rn_uni, aes(x = obs_rnd, y = uni)) +
  # keep plot layout
  geom_point(colour = "white") +
  geom_hline(data = lin_uni, aes(yintercept = yintercept), lty = 2, colour = c("#fd8d3c", "#a50f15")) +
  # half violin
  geom_half_violin(aes(fill = grp), side = "r", colour = NA, scale = "count", alpha = 0.4) +
  #geom_half_point(aes(colour = grp), side = "r", alpha = 0.1) +
  # pointrange
  geom_pointrange(data = rang_uni, aes(ymin = ymin, ymax = ymax), pch = 15, colour = c("#fd8d3c", "#fd8d3c", "#a50f15", "#a50f15"), lwd = 1) +
  scale_colour_manual(values = c("#fd8d3c", "#a50f15")) +
  scale_fill_manual(values = c("#fd8d3c", "#a50f15")) +
  scale_x_discrete(labels = c("Observed", "Random")) +
  labs(y = "Percent unique volume") +
  facet_wrap(~grp) +
  theme(legend.position = "none",
        axis.title.x = element_blank())
...
vol_diff <- dplyr::bind_rows(rn_used %>% dplyr::mutate(grp = "Used"), rn_thrt %>% dplyr::mutate(grp = "Extinction risk & use\nconsidered threat")) %>% 
  dplyr::mutate(grp = factor(grp, levels = c("Used", "Extinction risk & use\nconsidered threat"))) %>% 
  dplyr::mutate(diff_vol = obs_vol - rnd_vol)

rang_diff_vol <- vol_diff %>% 
  dplyr::group_by(grp) %>% 
  dplyr::summarise(median_diff = median(diff_vol),
                   ymin = quantile(diff_vol, 0.025),
                   ymax = quantile(diff_vol, 0.975))

vol_diff_plot <- ggplot(data = vol_diff, aes(x = "Used", y = diff_vol)) +
  # keep plot layout
  geom_point(colour = "white") +
  geom_hline(aes(yintercept = 0), lty = 2, lwd = 1, colour = "darkgrey") +
  # half violin
  geom_half_violin(data = vol_diff, aes(fill = grp), side = "r", colour = NA, scale = "count", alpha = 0.4) +
  # pointrange
  geom_pointrange(data = rang_diff_vol, aes(y = median_diff, ymin = ymin, ymax = ymax), pch = 15, colour = c("#fd8d3c", "#a50f15"), lwd = 1) +
  scale_colour_manual(values = c("#fd8d3c", "#a50f15")) +
  scale_fill_manual(values = c("#fd8d3c", "#a50f15")) +
  labs(y = bquote(atop("Difference in volume "~(SD^5), "compared to random"))) +
  facet_wrap(~grp) +
  theme(legend.position = "none",
        axis.title.x = element_blank(),
...
uni_diff <- vol_diff %>% 
  dplyr::mutate(diff_uni = (frac_unique_1 - frac_unique_2) * 100)

rang_diff_uni <- uni_diff %>% 
  dplyr::group_by(grp) %>% 
  dplyr::summarise(median_diff = median(diff_uni),
                   ymin = quantile(diff_uni, 0.025),
                   ymax = quantile(diff_uni, 0.975))

uni_diff_plot <- ggplot(data = uni_diff, aes(x = "Used", y = diff_uni)) +
  # keep plot layout
  geom_point(colour = "white") +
  geom_hline(aes(yintercept = 0), lty = 2, lwd = 1, colour = "darkgrey") +
  # half violin
  geom_half_violin(data = uni_diff, aes(fill = grp), side = "r", colour = NA, scale = "count", alpha = 0.4) +
  # pointrange
  geom_pointrange(data = rang_diff_uni, aes(y = median_diff, ymin = ymin, ymax = ymax), pch = 15, colour = c("#fd8d3c", "#a50f15"), lwd = 1) +
  scale_colour_manual(values = c("#fd8d3c", "#a50f15")) +
  scale_fill_manual(values = c("#fd8d3c", "#a50f15")) +
  labs(y = bquote(atop("Difference in unique volume (%)", "compared to random"))) +
  facet_wrap(~grp) +
  theme(legend.position = "none",
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
# # combine Fig 4 - instead see lower down with comparator volumes
# 
# fig4 <- cowplot::plot_grid(pca_used, NULL, cowplot::plot_grid(vol_plot, uni_plot, labels = c("B", "C"), nrow = 2), ncol = 3, rel_widths = c(1, 0.1, 1), labels = c("A", "", ""))
# 
# # cowplot::save_plot("outputs/fig4_glob.png", fig4, base_width = 12, base_height = 6, dpi = 300)
...
pred_prey_spp <- read.csv("data/comparator_iucn_preyspecies_nodup_withoutssp.csv", stringsAsFactors = FALSE)
...
# 
# saveRDS(hyp_pred, "data/hyp_pred.rds")

hyp_pred <- readRDS("data/hyp_pred.rds")
...
hum_spp <- read.csv("data/predatorHuman_species_list_withoutssp.csv", stringsAsFactors = FALSE)

# food

hum_food <- dplyr::filter(hum_spp, uTID19 == TRUE)

# hyp_hum_food <- lapply(hyp_vol_df$binomial, hyp_pred_prey, df = hum_food)
# 
# saveRDS(hyp_hum_food, "data/hyp_hum_food.rds")

hyp_hum_food <- readRDS("data/hyp_hum_food.rds")

hyp_hum_food_names <- sapply(1:length(hyp_hum_food), function(i) hyp_hum_food[[i]][[1]])
hyp_hum_food_vol <- sapply(1:length(hyp_hum_food), function(i) hyp_hum_food[[i]][[4]]@Volume)

hyp_hum_food_perc_match <- lapply(1:length(hyp_hum_food), function(i) hyp_hum_food[[i]][[2]]) %>% 
  dplyr::bind_rows() %>% 
  dplyr::mutate(binomial = hyp_hum_food_names) %>% 
  dplyr::mutate(perc = match_spp / prey_spp)

hyp_hum_food_vol_df <- data.frame(binomial = hyp_hum_food_names, vol = hyp_hum_food_vol, stringsAsFactors = FALSE) %>% 
  dplyr::filter(binomial %in% hyp_hum_food_perc_match$binomial)

# all

# hyp_hum <- lapply(hyp_vol_df$binomial, hyp_pred_prey, df = hum_spp)
# 
# saveRDS(hyp_hum, "data/hyp_hum.rds")

hyp_hum <- readRDS("data/hyp_hum.rds")
...
hypc_dumb_plot <- ggplot(data = hypc_dumb, aes(x = div, y = common_name)) +
  geom_line(aes(group = pair)) +
  geom_point(aes(color = type), size = 3) +
  geom_text(data = hypc_pred, aes(x = div - 30, label = ifelse(div < 10, sprintf("%.1f", round(div, digits = 1)), round(div, digits = 0))), colour = "#CC79A7") +
  geom_text(data = hypc_hum, aes(x = div + 30, label = ifelse(div < 10, sprintf("%.1f", round(div, digits = 1)), round(div, digits = 0))), colour = "#0072B2") +  
  ...
  annotation_custom(grid::rasterGrob(cuon), xmin = -90, xmax = -50, ymin = 6.6, ymax = 7.4) +
  ...
  annotation_custom(grid::rasterGrob(jaguar), xmin = -105, xmax = -45, ymin = 3.5, ymax = 4.5) +
  ...
  labs(x = bquote("Prey volume "~(SD^5)), colour = "") +
  scale_colour_manual(values = c("#CC79A7", "#0072B2"), labels = c("Predator", "Humans - all uses")) +
  scale_x_continuous(breaks = c(0, 200, 400, 600, 800), limits = c(-80, 820)) +
  theme(axis.title.y = element_blank(),
        legend.position = c(0.8, 0.5))

# combine Fig 4 - with comparator
...
# cowplot::save_plot("outputs/fig4_glob_cmp3_600.png", fig4_cmp, base_width = 12, base_height = 12, dpi = 600)

# fig4_cmp_diff <- cowplot::plot_grid(pca_used, NULL, cowplot::plot_grid(NULL, vol_diff_plot, NULL, NULL, uni_diff_plot, labels = c("B", "", "", "C", ""), nrow = 5, rel_heights = c(0.04, 1, 0.04, 0.04, 1)), ncol = 3, rel_widths = c(1, 0.1, 1), labels = c("A", "", "")) %>% 
#   cowplot::plot_grid(., cowplot::plot_grid(NULL, cowplot::plot_grid(NULL, hypc_dumb_plot, nrow = 2, rel_heights = c(0.04, 1)), NULL, ncol = 3, rel_widths = c(0.04, 1, 0.04)), nrow = 2, labels = c("", "D"))
# 
# # cowplot::save_plot("outputs/fig4_glob_cmp_diff.png", fig4_cmp_diff, base_width = 12, base_height = 12, dpi = 300)
...
tri_plot_bm <- ggplot(tri$dens, aes(trait, qmean, group = used)) +
  # ribbon of uncertainty across trait imputation datasets
  geom_ribbon(aes(ymin = qymin, ymax = qymax, fill = used)) +
  # line of mean across datasets
  geom_line(aes(colour = used), lwd = 1) +
  # # means uncertainty
  # geom_rect(data = tri$mean, aes(xmin = mean_min, xmax = mean_max, ymin = 0, ymax = (1.05 * (max(tri$dens$qymax))), fill = used), inherit.aes = FALSE, show.legend = FALSE) +
  # means
  geom_vline(data = tri$mean, aes(xintercept = mean, colour = used), size = 1.2, linetype = "dashed", show.legend = FALSE) +
  annotate("text", x = 4, y = 0.6, label = eff_txt, size = 4) +
  scale_color_manual(values = c("grey", "#fd8d3c")) +
  scale_fill_manual(values = c("lightgrey", "#fdbe85")) +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0), limits = c(0, (1.05 * (max(tri$dens$qymax))))) +
  labs(x = expression(log[10](Body~mass)), y = "Density") +
  facet_wrap(~ title) +
...
tri_plot_ls <- ggplot(tri$dens, aes(trait, qmean, group = used)) +
  # ribbon of uncertainty across trait imputation datasets
  geom_ribbon(aes(ymin = qymin, ymax = qymax, fill = used)) +
  # line of mean across datasets
  geom_line(aes(colour = used), lwd = 1) +
  ...
  # means
  geom_vline(data = tri$mean, aes(xintercept = mean, colour = used), size = 1.2, linetype = "dashed", show.legend = FALSE) +
  annotate("text", x = 3.6, y = 0.8, label = eff_txt, size = 4) +
  scale_color_manual(values = c("grey", "#fd8d3c")) +
  scale_fill_manual(values = c("lightgrey", "#fdbe85")) +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0), limits = c(0, (1.05 * (max(tri$dens$qymax))))) +
  labs(x = expression(sqrt(Litter/clutch~size)), y = "Density") +
  facet_wrap(~ title) +
  theme(legend.title = element_blank())
...
tri_plot_hb <- ggplot(tri$dens, aes(trait, qmean, group = used)) +
  # ribbon of uncertainty across trait imputation datasets
  geom_ribbon(aes(ymin = qymin, ymax = qymax, fill = used)) +
  # line of mean across datasets
  geom_line(aes(colour = used), lwd = 1) +
  ...
  # means
  geom_vline(data = tri$mean, aes(xintercept = mean, colour = used), size = 1.2, linetype = "dashed", show.legend = FALSE) +
  annotate("text", x = 3.6, y = 1, label = eff_txt, size = 4) +
  scale_color_manual(values = c("grey", "#fd8d3c")) +
  scale_fill_manual(values = c("lightgrey", "#fdbe85")) +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0), limits = c(0, (1.05 * (max(tri$dens$qymax))))) +
  labs(x = expression(sqrt(Habitat~breadth)), y = "Density") +
  facet_wrap(~ title) +
  theme(legend.title = element_blank())
...
tri_plot_gl <- ggplot(tri$dens, aes(trait, qmean, group = used)) +
  # ribbon of uncertainty across trait imputation datasets
  geom_ribbon(aes(ymin = qymin, ymax = qymax, fill = used)) +
  # line of mean across datasets
  geom_line(aes(colour = used), lwd = 1) +
  ...
  # means
  geom_vline(data = tri$mean, aes(xintercept = mean, colour = used), size = 1.2, linetype = "dashed", show.legend = FALSE) +
  annotate("text", x = 1.05, y = 2, label = eff_txt, size = 4) +
  scale_color_manual(values = c("grey", "#fd8d3c")) +
  scale_fill_manual(values = c("lightgrey", "#fdbe85")) +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0), limits = c(0, (1.05 * (max(tri$dens$qymax))))) +
  labs(x = expression(log[10](Generation~length)), y = "Density") +
  facet_wrap(~ title) +
  theme(legend.title = element_blank())
...
tri_plot_diet <- ggplot(tri$dens, aes(trait, qmean, group = used)) +
  # ribbon of uncertainty across trait imputation datasets
  geom_ribbon(aes(ymin = qymin, ymax = qymax, fill = used)) +
  # line of mean across datasets
  geom_line(aes(colour = used), lwd = 1) +
  ...
  # means
  geom_vline(data = tri$mean, aes(xintercept = mean, colour = used), size = 1.2, linetype = "dashed", show.legend = FALSE) +
  annotate("text", x = -0.1, y = 7.5, label = eff_txt, size = 4) +
  scale_color_manual(values = c("grey", "#fd8d3c")) +
  scale_fill_manual(values = c("lightgrey", "#fdbe85")) +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0), limits = c(0, (1.05 * (max(tri$dens$qymax))))) +
  labs(x = "Diet", y = "Density") +
  facet_wrap(~ title) +
  theme(legend.title = element_blank())

# combine plots
tri_plot_comb <- cowplot::plot_grid(tri_plot_bm + theme(legend.position = "none"), tri_plot_gl + theme(legend.position = "none"), tri_plot_diet + theme(legend.position = "none"), tri_plot_hb + theme(legend.position = "none"), tri_plot_ls + theme(legend.position = "none"), cowplot::get_legend(tri_plot_bm))

# cowplot::save_plot("outputs/traits_ind_eff.png", tri_plot_comb, base_height = 6, base_width = 12)
